...
> 🚀 这是我在 B 站课程《接口测试的简介与分类》（[点击直达](https://www.bilibili.com/video/BV11K4y1J7sh?vd_source=65c2968c09490f4b218154711916b4d8)）学习过程中的详细笔记与实操总结，面向接口测试初学者与自动化测试入门者，希望对你有所启发。
...
* 环境变量：限定于特定环境（如 dev/test/prod），variable 命名必须一致，接口地址中使用 {{variable命名}}。
...
| 参数                 | 格式示例                     | 典型用途               |
| ------------------ | ------------------------ | ------------------ |
| {{\$timestamp}}    | 1689345678（秒级）           | 生成当前时间戳（如签名计算）     |
| {{\$randomDate}}   | 2025-06-18T12:34:56Z     | 随机日期（测试订单有效期）      |
| {{\$isoTimestamp}} | 2025-06-18T12:34:56.789Z | 符合 ISO 8601 格式的时间戳 |
...
| 参数                 | 格式示例                                 | 典型用途                           |
| ------------------ | ------------------------------------ | ------------------------------ |
| {{\$randomInt}}    | 345（0-1000 内随机数）                     | 测试分页大小（如 size={{\$randomInt}}） |
| {{\$randomString}} | abcdef12345（16 位随机字符串）               | 生成随机用户名                        |
| {{\$guid}}         | 5f7d3a9c-1b2e-4c3d-8e9f-0a1b2c3d4e5f | 唯一标识符（测试分布式 ID）                |
...
| 参数                | 格式示例                                              | 典型用途                  |
| ----------------- | ------------------------------------------------- | --------------------- |
| {{\$email}}       | [user567@example.com](mailto:user567@example.com) | 注册测试（随机邮箱）            |
| {{\$phoneNumber}} | +1 (555) 123-4567                                 | 测试短信验证码接口             |
| {{\$creditCard}}  | 4111 1111 1111 1111                               | 支付模拟（需配合 Mock Server） |
